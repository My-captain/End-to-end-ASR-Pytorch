...
class Solver(BaseSolver):
    ...
    def load_data(self):
        ...
        self.tr_set, self.dv_set, self.feat_dim, self.vocab_size, self.tokenizer, msg = \
            load_dataset(self.paras.njobs, self.paras.gpu, self.paras.pin_memory, self.curriculum > 0, **self.config['data'])
        self.verbose(msg)
    ...
    def exec(self):
        ...
        while self.step < self.max_step:
            ...
            if 0 < self.curriculum == n_epochs:
                self.verbose('Curriculum learning ends after {} epochs, starting random sampling.'.format(n_epochs))
                self.tr_set, _, _, _, _, _ = load_dataset(self.paras.njobs, self.paras.gpu, self.paras.pin_memory, False, **self.config['data'])
            for data in self.tr_set:
                ...
                try:
                    ctc_output, encode_len, att_output, att_align, dec_state = self.model(feat, feat_len, max(txt_len), tf_rate=tf_rate, teacher=txt, get_dec_state=self.emb_reg)
                ...
                if ctc_output is not None:
                    if self.paras.cudnn_ctc:
                        ctc_loss = self.ctc_loss(ctc_output.transpose(0, 1),
                                                 ...
                                                 [ctc_output.shape[1]] * len(ctc_output), txt_len.cpu().tolist())
                    else:
                        ctc_loss = self.ctc_loss(ctc_output.transpose(0, 1), txt, encode_len, txt_len)
                        if math.isinf(ctc_loss):
                            print("a")
                ...
                if (self.step == 1) or (self.step % self.PROGRESS_STEP == 0):
                    self.progress('Tr stat | Loss - {:.2f} | Grad. Norm - {:.2f} | {}'.format(total_loss.cpu().item(), grad_norm, self.timer.show()))
                    self.write_log('loss', {'tr_ctc': ctc_loss, 'tr_att': att_loss})
                    self.write_log('emb_loss', {'tr': emb_loss})
                    self.write_log('wer',
                                   {'tr_att': cal_er(self.tokenizer, att_output, txt),
                ...
                del ctc_output, encode_len, att_output, att_align, dec_state, total_loss, grad_norm, ctc_loss, att_loss
    ...
    def validate(self):
        ...
        for i, data in enumerate(self.dv_set):
            ...
            # Forward model
            with torch.no_grad():
                ctc_output, encode_len, att_output, att_align, dec_state = self.model(feat, feat_len, int(max(txt_len)*self.DEV_STEP_RATIO), emb_decoder=self.emb_decoder)
            ...
            if i % 800 == 0:
                try:
                    for j in range(min(len(txt), self.DEV_N_EXAMPLE)):
                        filename = data[0][j]
                        if self.step < 10:
                            ground_truth = self.tokenizer.decode(txt[j].tolist())
                            self.write_log(f'text{filename}_0', ground_truth)
                        if att_output is not None:
                            att_text = self.tokenizer.decode(att_output[j].argmax(dim=-1).tolist(), debug=True)
                            self.write_log(f'align{filename}_align', feat_to_fig(att_align[j, 0, :, :encode_len[j]].cpu().detach(), att_text.split(" ")))
                            self.write_log(f'text{filename}_att', att_text)
                        if ctc_output is not None:
                            self.write_log(f'text{filename}_ctc', self.tokenizer.decode(ctc_output[j].argmax(dim=-1).tolist(), ignore_repeat=True, debug=True))
                except Exception as e:
                    print(repr(e))
            # """
            ...
            #         if ctc_output is not None:
            #             self.write_log('ctc_text{}'.format(i), self.tokenizer.decode(ctc_output[i].argmax(dim=-1).tolist(), ignore_repeat=True))
    ...
    def inference(self, song_path):
        from src.audio import create_transform
        audio_transform, _ = create_transform(self.config["data"]["audio"])
        feat = audio_transform(song_path).to(self.device)
        feat_len = len(feat)
