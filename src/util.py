...
def feat_to_fig(feat, txt=None):
    # feat TxD tensor
    data = _save_canvas(feat.numpy(), txt=txt)
    return torch.FloatTensor(data), "HWC"


def _save_canvas(data, meta=None, txt=None):
    fig, ax = plt.subplots(figsize=(19, 10))
    fig.subplots_adjust(left=0.05, bottom=0.03, right=0.98, top=0.98)
    if meta is None:
        if txt is not None:
...
if __name__ == '__main__':
    data = dill.load(open("../data.dill", mode="rb"))
    _save_canvas(data)
