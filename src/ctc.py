    ...
    def add_token(self, token, ctc_token_t, Pr_k_y=None):
        ...
        if len(self.y) == 0:
            ...
            Pr_y_t_nblank_new = ctc_token_t + lm_prob + (self.Pr_y_t_blank_bkup if self.y[-1] == token else \
                                                             np.logaddexp(self.Pr_y_t_blank_bkup,
                                                                          self.Pr_y_t_nblank_bkup))
...
class CTCBeamDecoder(nn.Module):
    ...
    def forward(self, feat, feat_len):
        ...
        if self.apply_lm:
            # 0 == <sos> for RNNLM
            output, hidden = \
                self.lm(torch.zeros((1, 1), dtype=torch.long).to(self.device),
                        torch.ones(1, dtype=torch.long).to(self.device), None)
            B[0].update_lm(
        ...
        for t in range(T):
            ...
            for i in range(len(B)):  # For y in B
                ...
                if self.apply_lm:
                    ...
                    ctc_vocab_cand = sorted(zip(self.vocab_range, ctc_output[t, self.vocab_range]), reverse=True,
                                            key=lambda x: x[1])
