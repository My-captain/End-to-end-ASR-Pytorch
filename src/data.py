...
def collect_audio_batch(batch, audio_transform, mode):
    ...
    with torch.no_grad():
        for b in batch:
            file_name = "/".join(b[0].split('/')[-2:])
            try:
                feat = audio_transform(b[0])
            except Exception as e:
                print(f"audio_transform error: {b[0]}")
                continue
            if len(feat) // 4 < len(b[1]):
                # 卷积后的特征长度小于文本长度,造成CTC LOSS为INFINITE
                print(f"feature too short: {b[0]}")
                continue
            if len(feat) > MAX_AUDIO_LEN:
                # CUDA OOM, 特征过大, 例如audio/尧十三 - 龙港秘密/20.wav、长度为25875
                print(f"feature too long: {b[0]}")
                continue
            txt_set = set(b[1])
            if len(b[1]) > 3 and len(txt_set) < 3:
                # 重复的,造成CTC LOSS为INFINITE
                print(f"Text all repeat: {b[0]}")
                continue
            ctc_txt = validate_ctc_length(b[1])
            if len(ctc_txt) > len(feat) // 4:
                # 重复的文字,造成CTC LOSS为INFINITE
                print(f"length of CTC text > feature length: {b[0]}\t{ctc_txt}\t{len(feat)//4}")
                continue
    ...
    audio_len, file, audio_feat, text = zip(*[(feat_len, f_name, feat, txt)
                                              for feat_len, f_name, feat, txt in sorted(zip(audio_len, file, audio_feat, text),
                                            reverse=True, key=lambda x:x[0])])

    first_len = audio_len[0]
    if first_len > HALF_BATCHSIZE_AUDIO_LEN and mode == 'train':
        print(f"batch折半: {first_len}")
        audio_len, file, audio_feat, text = audio_len[:len(batch)//2], file[:len(batch)//2], audio_feat[:len(batch)//2], text[:len(batch)//2]
    # Zero-padding
...
def validate_ctc_length(txt: list):
    txt = txt.copy()
    idx = len(txt) - 1
    while idx > 0:
...
def create_dataset(tokenizer, ascending, name, path, bucketing, batch_size,
    ...
    elif name.lower() == "chinapopsong":
        from corpus.chinaPopSong import ChinaPopAudioDataset as Dataset
    else:
...
def create_textset(tokenizer, train_split, dev_split, name, path, bucketing, batch_size):
    ...
    if name.lower() == "librispeech":
        ...
        from corpus.chinaPopSong import ChinaPopTextDataset as Dataset
    else:
        raise NotImplementedError
...
def load_textset(n_jobs, use_gpu, pin_memory, corpus, text):
    ...
    tr_set = DataLoader(tr_set, batch_size=tr_loader_bs, shuffle=True, drop_last=True, collate_fn=collect_tr, num_workers=n_jobs, pin_memory=use_gpu)
    dv_set = DataLoader(dv_set, batch_size=dv_loader_bs, shuffle=False, drop_last=False, collate_fn=collect_dv, num_workers=n_jobs, pin_memory=pin_memory)
