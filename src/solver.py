...
class BaseSolver:
    ...
    def load_ckpt(self):
        ...
        if self.paras.load:
            ...
            if self.mode == 'train':
                ...
                self.verbose('Load ckpt from {}, restarting at step {} (recorded {} = {:.2f} %)'.format(self.paras.load,
                                                                                                        self.step,
                                                                                                        metric, score))
            else:
                self.model.eval()
    ...
    def write_log(self, log_name, log_dict):
        ...
        if type(log_dict) is dict:
            log_dict = {key: val for key, val in log_dict.items()
        ...
        if log_dict is None:
            ...
            if 'align' in log_name or 'spec' in log_name:
                img, form = log_dict
                log_name = log_name.replace("align", "")
                self.log.add_image(log_name, img, global_step=self.step, dataformats=form)
            elif 'text' in log_name or 'hyp' in log_name:
                log_name = log_name.replace("text", "")
                self.log.add_text(log_name, log_dict, global_step=self.step)
            else:
                self.log.add_scalars(log_name, log_dict, global_step=self.step)
    ...
    def save_checkpoint(self, f_name, metric, score, show_msg=True):
        ...
        if show_msg:
            self.verbose(
                "Saved checkpoint (step = {}, {} = {:.5f}) and status @ {}".format(human_format(self.step), metric, score, ckpt_path))
