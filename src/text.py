...
class _BaseTextEncoder(abc.ABC):
    ...
    @abc.abstractmethod
    def decode(self, ids, ignore_repeat=False, debug=False):
        raise NotImplementedError
...
class CharacterTextEncoder(_BaseTextEncoder):
    ...
    def decode(self, idxs, ignore_repeat=False, debug=False):
        vocabs = []
...
class WordTextEncoder(CharacterTextEncoder):
    ...
    def decode(self, idxs, ignore_repeat=False, debug=False):
        ...
        for t, idx in enumerate(idxs):
            v = self.idx_to_vocab(idx)
            if idx == self.eos_idx:     # <EOS>
                if debug:
                    vocabs.append(v)
                break
            elif idx == self.pad_idx:   # <PAD>
                if debug:
                    vocabs.append(v)
                else:
                    continue
            elif ignore_repeat and t > 0 and idx == idxs[t-1]:  # CTC解码, 当前与前一个重复
                if debug:
                    # CTC解码、debug模式
                    vocabs.append("+")
                continue
